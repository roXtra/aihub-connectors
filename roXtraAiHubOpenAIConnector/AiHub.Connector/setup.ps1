...
.EXAMPLE
pwsh -File .\setup.ps1 -ServiceName roXtraAiHubOpenAIConnector -InstallDir "C:\\Program Files\\roXtraAiHubOpenAIConnector"
...
param (
	[string]$ServiceName = "roXtraAiHubOpenAIConnector",
	[string]$DisplayName = "roXtra AI Hub OpenAI Connector",
	[string]$Description = "roXtra AI Hub OpenAI Connector Windows Service",
	[string]$InstallDir = (Join-Path $env:ProgramFiles 'roXtraAiHubOpenAIConnector'),
	# Executable name inside the artifact/install directory
	[string]$ExeName = 'AiHub.Connector.OpenAI.exe'
)
