...
{
	private readonly IExternalConnector _connector;
					...
					case var t when t == KnowledgePoolFileAddedEvent.EventType:
						return await HandleEventAsync<KnowledgePoolFileAddedEvent>(
								...
								async (evt, ct) =>
								{
									...
									var file = await DownloadFileFromEventAsync(
											evt.FileId,
											evt.Title,
											evt.DownloadUrl,
											evt.SupportedForKnowledgePools,
											evt.DocumentHash,
											ct
										)
									...
									{
										await _connector.HandleKnowledgePoolFileAddedAsync(evt.KnowledgePoolId, file, ct).ConfigureAwait(false);
									}
					...
					case var t when t == KnowledgePoolCreatedEvent.EventType:
						return await HandleEventAsync<KnowledgePoolCreatedEvent>(
								...
								async (evt, ct) =>
								{
									...
									await _connector.HandleKnowledgePoolCreatedAsync(evt.KnowledgePoolId, ct).ConfigureAwait(false);
									return Results.Ok(
										new
										{
											status = "processed",
					...
					case var t when t == KnowledgePoolFileRemovedEvent.EventType:
						return await HandleEventAsync<KnowledgePoolFileRemovedEvent>(
								...
								async (evt, ct) =>
								{
									...
									await _connector.HandleKnowledgePoolFileRemovedAsync(evt.KnowledgePoolId, evt.FileId, ct).ConfigureAwait(false);
									return Results.Ok(
										new
										{
											status = "processed",
					...
					case var t when t == KnowledgePoolRemovedEvent.EventType:
						return await HandleEventAsync<KnowledgePoolRemovedEvent>(
								...
								async (evt, ct) =>
								{
									...
									await _connector.HandleKnowledgePoolRemovedAsync(evt.KnowledgePoolId, ct).ConfigureAwait(false);
									return Results.Ok(
										new
										{
											status = "processed",
	...
	private async Task<RoxFile> DownloadFileFromEventAsync(
		string fileId,
		string title,
		string downloadUrl,
		bool includeContent,
	...
	)
	{
		...
		if (!includeContent)
		{
			return new RoxFile(fileId, title) { DocumentHash = documentHash };
		}
		...
		return new RoxFile(fileId, title) { ContentStream = responseStream, DocumentHash = documentHash };
	}
}
